...
JBlock::JBlock(Board *b,int level) : Block{b, level} {
	blockType = 'J';
	...
	if(!isCoordsFreeOnBoard()) throw out_of_range{"J"};

	updateBoard();
}

void JBlock::config1ToConfig2() {
	theCoords[0].setX(theCoords[0].getX()-1);
	theCoords[0].setY(theCoords[0].getY()+1);
	theCoords[1].setX(theCoords[1].getX()-2);
